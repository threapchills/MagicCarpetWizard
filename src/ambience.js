...
export class AmbientEngine {
  ...
  setEnabled(enabled) {
    ...
    else for (const layer of this.layers.values()) layer.next = 0;
  }
  stop() {
  ...
  spawn(layer, buffer) {
    ...
    envelope.gain.setValueAtTime(0, now); envelope.gain.linearRampToValueAtTime(.72, now + fade);
    envelope.gain.setValueAtTime(.72, now + length - fade); envelope.gain.linearRampToValueAtTime(0, now + length);
